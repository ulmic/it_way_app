...
import 'package:it_way_app/audio_player/features/media_player/audio_player_task.dart';
...
class MediaPlayerCubit extends Cubit<MediaPlayerStateAbstract> {
  ...
  void _addTrackAndPlay(AudioTrack track) {
    ...
    AudioService.customAction(
        AudioPlayerTask.PLAY_ACTION, playingSpeedBloc.playingSpeed);
  }
  ...
  Future<void> play(AudioTrack track) async {
    if (AudioService.running) {
      await nextTrack(track);
    } else {
      emit(MediaPlayerLoadingTrackState(null));

      if (_audioTrack == null) {
        _audioTrack = await getAudioTrackUseCase.next(track: track);
  ...
  Future<void> nextTrack(AudioTrack track) async {
    if (_audioTrack is AudioTrack) {
      var nextTrack = await getAudioTrackUseCase.next(track: track);
  ...
  Future<void> stop() async {
    ...
    await _addActionToAudioService(
